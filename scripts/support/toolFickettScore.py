...
def main(input_file, output_file):
    ids, sequences = readfile(input_file)
    with open(output_file, 'w') as f:
        f.write("Sequence_ID,Fickett_Score,Call_Label\n")
        for id, seq in zip(ids, sequences):
            fickett_score = ficketTestcode(seq)
            if fickett_score < .74:
                call_label = "non-coding"
            elif fickett_score >= .74 and fickett_score < .95:
                call_label = "No decision"
            else:
                call_label = "protein-coding"
            
            f.write(f"{id},{fickett_score:.4f},{call_label}\n")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Calculate Fickett Score for sequences in a FASTA file.')
